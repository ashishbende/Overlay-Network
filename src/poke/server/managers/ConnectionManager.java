...
public class ConnectionManager {
	...
	private static HashMap<Integer, Channel> localConnections = new HashMap<Integer, Channel>();
	public static enum connectionState {SERVERAPP, SERVERMGMT, CLIENTAPP, LOCALSERVERAPP };
	...
	public static void addConnection(Integer nodeId, Channel channel, connectionState state) {
		...
		else if(state == connectionState.LOCALSERVERAPP){
			localConnections.put(nodeId, channel);
			logger.info("Local cluster Server Added in Connection Manager " + nodeId);
		}
	}
	
	public static HashMap<Integer, Channel> getallconnections(){
	...
	public static Channel getConnection(Integer nodeId, connectionState state) {
				...
				{
					logger.info("Local server node got in Connection Manager " + nodeId);
					return localConnections.get(nodeId);
				}
				return null;
	...
	public synchronized static void removeConnection(Integer nodeId, connectionState state) {
		if (state == connectionState.SERVERMGMT)
			...
			localConnections.remove(nodeId);
	}
	
	public synchronized static void removeAllSeverAppConnection(){
		HashMap<Integer, Channel> severappconnections = new HashMap<Integer, Channel>();
		appconnections = severappconnections;
	}
	...
	public synchronized static void removeConnection(Channel channel, connectionState state) {
		...
		{
			if (!localConnections.containsValue(channel)) {
				return;
			}

			for (Integer nid : localConnections.keySet()) {
				if (channel == localConnections.get(nid)) {
					localConnections.remove(nid);
					break;
				}
			}
	...
	public synchronized static void broadcastToAllLocalNodes(Request req) {
		if (req == null)
			return;

		for (Channel ch : localConnections.values())
			ch.writeAndFlush(req);
	}
	...
	public synchronized static void sendToLocalNode(Request req,
			Integer destination){
		if (req == null)
			return;
		if (localConnections.get(destination) != null){
			localConnections.get(destination).writeAndFlush(req);
		} else
			System.out.println("No local node destination found");
	}
	...
	public static Set<Integer> getKeySetConnections(connectionState state){
		...
		return connections.keySet();
			...
			return localConnections.keySet();
		else 
			return null;
	}
	public static boolean checkClient(connectionState state, int key)
	{
		if(state == connectionState.CLIENTAPP)
			...
			return localConnections.containsKey(key);
		else 
				return false;
	}
}
